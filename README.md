...
- 2.4G 回包有约 2~5s 固有延迟（RF 时隙），键盘睡眠时无回包
...
- **空闲门控**：`GetLastInputInfo` 检测系统键鼠输入，空闲 ≥1.2s 才发帧，等不到 6s 放弃
- **按需查询**：无定时轮询，仅启动时 / 打开面板 / 手动刷新时查询，20s 新鲜期内不重查
- 单次查询最多 2 帧，查询完立即释放 HID 句柄
